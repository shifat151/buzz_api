...
class registrationForm(forms.Form):
    ...
    experience_in_months = forms.IntegerField(max_value=100, min_value=0, required=False, label="Experience in months: ",
                                              help_text="Enter your professional working experience in months.")
    current_work_place_name = forms.CharField(max_length=256, required=False, label="Current work place name: ")
    apply_choice = [
        ('Mobile', 'Mobile'),
        ('Backend', 'Backend')
    ]
    applying_in = forms.ChoiceField(choices=apply_choice, label='Applying in: ')
    expected_salary = forms.IntegerField(min_value=15000, max_value=60000, label="Expected Salary: ", help_text="Enter your expected salary between 15000 and 60000.")
